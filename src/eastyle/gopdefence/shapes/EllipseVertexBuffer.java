package eastyle.gopdefence.shapes;
	...
	void update(int segments, float width, float height, int filledMode) {
		...
		case GL10.GL_LINE_LOOP:
			for (float i = 0; i < 360.0f; i += (360.0f / segments)) {
				vertices[count++] = Float.floatToRawIntBits((float) (Math
						.cos(MathUtils.degToRad(i)) * width));
				vertices[count++] = Float.floatToRawIntBits((float) (Math
						.sin(MathUtils.degToRad(i)) * height));
			}
			break;
		...
		case GL10.GL_TRIANGLE_FAN:
			for (float i = 0; i < 360.0f; i += (360.0f / segments)) {
				vertices[count++] = Float.floatToRawIntBits((float) (Math
						.cos(MathUtils.degToRad(360 - i)) * width));
				vertices[count++] = Float.floatToRawIntBits((float) (Math
						.sin(MathUtils.degToRad(360 - i)) * height));
			}
			break;
		}
